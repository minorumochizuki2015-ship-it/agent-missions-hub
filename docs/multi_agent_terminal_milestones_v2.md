...
- **チャット連携タスク（Phase3+）**: chat/ConPTY/attach PoC は完了（単一ロール stream＋人間 attach、`orchestrator_chat_run`/`orchestrator_chat_attach` を ci_evidence 記録）。ロール間 handoff 拡張・複数ロール chat は後続。テスト: `tests/test_conpty_stream.py` / `tests/test_chat_attach.py`。
- **attach/multi-role 拡張方針（Phase3+）**: attach 最小機能を完了済み（TTY attach）。今後は Message Bus を用いた handoff 拡張と multi-role chat（Sequential から着手、並列は後続）を B/T レーン小分けで進める。
