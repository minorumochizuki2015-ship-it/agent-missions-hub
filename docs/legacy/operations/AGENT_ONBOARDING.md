...
### Workspace Verification Checklist (Critical Incident Prevention)

Before running any command or issuing a report, complete this lightweight checklist to prevent cross-workspace mistakes:

1. **Confirm current directory**: Run `Get-Location` (or `pwd` in bash) and ensure it matches the project you intend to inspect (e.g., `C:\Users\User\Trae\Codex-CLImutipule-CMD`). Paste the result into your notes if the workspace was recently switched.
2. **Validate required files**: Use `Test-Path <file>` or `Get-ChildItem` to confirm the exact files you plan to reference (package.json, tests, logs, etc.) exist in this workspace before summarizing them.
3. **Capture evidence before conclusions**: When you need to mention file content, run `Get-Content`/`Select-String` first, then cite the command output. Never rely on memory or previous runs.
4. **Re-check prior to reporting**: Immediately before sending a report, rerun `Get-Location` and sanity-check that the files you cite belong to that directory. This step catches late context switches.

Failing to follow the above sequence caused a critical incident on 2025-11-10. Treat these steps as mandatory for all audits and status reports.

### File Verification Checklist After Errors

If any `Get-Content`/`Test-Path`/`type` command fails:

1. **List the directory immediately** with `Get-ChildItem` (or `dir`) to confirm the file truly exists in the current workspace.
2. **Re-run `Test-Path`** on the exact path copied from the directory listing; only after that should you conclude whether the path is invalid.
3. **Avoid speculative reports**: if a file read fails, gather the directory listing and the exact command output before writing “not found.” Include both in the audit log so reviewers can see the evidence.
4. **Only proceed once both workspace and file existence are re-confirmed.**

Skipping these checks caused false “file missing” reports during the Phase 7 audit; follow this checklist to prevent recurrence.
