...
1. **Read:** `docs/external/GUIDE_TO_OPTIMAL_MCP_SERVER_DESIGN.md` (sections 3–7). Highlight
...
5. **Security testing:** Add the MSB prompt-attack suite to your CI/CD gate (see
   `docs/external/GUIDE_TO_OPTIMAL_MCP_SERVER_DESIGN.md`, section 7). Record Net Resilient
...
2. **Log shipping:** Follow `docs/external/observability.md` to push JSON logs into Loki
   (or your chosen sink).
...
4. **Dashboards:** Build panels for `calls`, `errors`, and `capabilities` by
   ...
   snippets are provided in `docs/external/observability.md`.
