...
@pytest.mark.asyncio
async def test_ui_gate_server_start(start_server):
    """UIサーバーが正常に起動することを確認"""
    # サーバーが既に起動しているはず（fixtureで起動済み）
...
async def test_ui_gate_unified_inbox_accessibility(start_server, server_url):
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            with open(AXE_RESULTS_PATH, "w", encoding="utf-8", newline="\n") as f:
                f.write(json.dumps(axe_results, ensure_ascii=False, indent=2) + "\n")
