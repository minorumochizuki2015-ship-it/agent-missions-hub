...
11. 本プロジェクトの diff 上限はレーン別に適用する：  
    ...
    - Tレーン（テスト専用: tests/** などテストコード・テストデータのみ）= 上限20ファイル/800行まで緩和。src/ や設定・ポリシー類に触れた場合は A/B レーン違反として扱う。テスト失敗時は即ロールバックし、ci_evidence へ rollback/理由を記録する。  
...
14. プロジェクト完了フェーズの最終目標は、強いゲート（Shadow Audit / APPROVALS / lanes / GitOps）を前提に、WORK/AUDIT/CMD が push / PR / （将来的に）auto-merge まで自動実行し、人間はルール・APPROVALS・waiver 設計に専念する状態とする。  
    - auto-push/auto-merge を実際に有効化してよいのは、少なくとも次をすべて満たす場合に限る：  
      - 許可ブランチ（例: `feature/*`）かつ lane 制約内（A=3files/50lines, B≤200lines/5files, T=tests専用, C=機械的）の diff であること  
      - Shadow Audit manifest/hash/signature が verify_chain=OK で PLAN/TEST/PATCH/APPLY/APPROVALS イベントが揃っていること  
      - 対象 HEAD について AUDIT レポートが `Status: Proceed`（少なくとも Blocking なし）であること  
      - `APPROVALS.md` に scope=auto-push および/または scope=auto-merge の行があり、two-person ルール（WORK 要求 / CMD or AUDIT 承認）が満たされていること  
      - `CODEX_GIT_MODE` が `auto-push` または `auto-push-pr`（将来的には `auto-merge` プロファイル）であり、`REMOTE_WRITE_ALLOWED=1` が有効であること  
    - 上記条件を満たさない場合、WORK は push/merge を自動実行してはならず、従来どおり人間または運用側の判断に委ねる。
