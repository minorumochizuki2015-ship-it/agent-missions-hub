...
- branch=feature/chat-attach。Bレーン: cli.py+conpty_stream.py 199行/2ファイル、Tレーン: tests/test_chat_attach.py 1ファイル。attach サブコマンドとセッションレジストリを追加し、ci_evidence に orchestrator_chat_attach を記録。pytest (chat_attach) 1 passed（PYTEST_DISABLE_PLUGIN_AUTOLOAD=1, addopts=）。Shadow Audit PLAN/TEST/PATCH/APPLY emit＋verify_chain=True。手動 attach ログを追加し、ci_evidence に run_id/mission_id/log_path_hash/git_sha 付き attach イベントを追記。
- branch=feature/safeops-manual-plan。Bレーン純増114行/2ファイル（cli.py/safeops.py）、Aレーン3行、Tレーン3ファイル。orchestrator run に PLAN/TEST/PATCH/APPLY スケルトンと trace 出力を追加し、automation_level=manual を明示。pytest allowlist3件=3 passed/2 skipped、Shadow Audit PLAN/TEST/PATCH/APPLY emit＆verify_chain=OK。shadow_audit_verify ユーティリティは未着手。
