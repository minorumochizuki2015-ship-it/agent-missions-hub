...
_OPEN_REPOS: set[Repo] = set()


def _register_repo(repo: Repo) -> Repo:
    _OPEN_REPOS.add(repo)
    return repo


def close_all_archives() -> None:
    for repo in list(_OPEN_REPOS):
        try:
            repo.close()
        except Exception:
            logging.debug("repo close failed during cleanup", exc_info=True)
        finally:
            _OPEN_REPOS.discard(repo)
...
class AsyncFileLock:
    ...
    async def __aenter__(self) -> None:
        ...
        try:
            ...
            log.warning("AsyncFileLock.acquire_timeout", path=str(self._path))
            if self._is_stale():
                log.info("AsyncFileLock.breaking_stale", path=str(self._path))
                with contextlib.suppress(Exception):
                    await _to_thread(self._break_stale)
                # Retry acquire after breaking a stale lock
                await _to_thread(self._lock.acquire, timeout=self._timeout)
                log.info("AsyncFileLock.acquire_retry_success", path=str(self._path))
            else:
                raise
        self._held = True
    ...
    async def __aexit__(self, exc_type, exc, tb) -> None:
        ...
        # Ensure leftover lock file is removed on Windows if release didn't clean it up
        with contextlib.suppress(Exception):
    ...
    def _is_stale(self) -> bool:
        """Determine whether the current lock file is stale.

        A lock is considered stale if:
        - Metadata exists and pid is not alive; or
        - Lock file mtime is older than stale_timeout_seconds.
        """
        try:
            lock_exists = self._path.exists()
            if not lock_exists:
                return False
            # Check metadata when present
            ...
            # On any error, be conservative and do not force-break
            return False
        return False

    def _break_stale(self) -> None:
        """Forcibly remove stale lock and its metadata."""
        with contextlib.suppress(Exception):
...
async def ensure_archive(settings: Settings, slug: str) -> ProjectArchive:
    ...
    log.info("ensure_archive.root_ensured", repo_root=str(repo_root))
    project_root = repo_root / "projects" / slug
    await _to_thread(project_root.mkdir, parents=True, exist_ok=True)
    # Ensure baseline subdirectories exist for tests and tools
    for sub in ("agents", "file_reservations", "messages"):
        await _to_thread((project_root / sub).mkdir, parents=True, exist_ok=True)
    log.info("ensure_archive.done", project_root=str(project_root))
...
async def _ensure_repo(root: Path, settings: Settings) -> Repo:
    git_dir = root / ".git"
    if git_dir.exists():
        return _register_repo(Repo(str(root)))
...
async def _write_text(path: Path, content: str) -> None:
    await _to_thread(path.parent.mkdir, parents=True, exist_ok=True)
    # Enforce LF line endings on all platforms to satisfy CI EOL policy
