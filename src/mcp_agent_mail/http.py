...
__all__ = ["app", "build_http_app", "main"]
...
class BearerAuthMiddleware(BaseHTTPMiddleware):
    ...
    class HealthCheckBypassMiddleware(BaseHTTPMiddleware):
        def __init__(self, app: FastAPI, base_paths: list[str]) -> None:
            super().__init__(app)
            self._base_paths = set(base_paths)
...
class SecurityAndRateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint):
        ...
        # Skip this middleware entirely for MCP endpoints to avoid stream issues
        path = request.url.path
        if path.startswith("/mcp") or path == "/mcp" or "/mcp/" in path:
            return await call_next(request)

        # For non-MCP endpoints, proceed with auth/rate limiting
        body_bytes = b""
        ...
        if self.settings.http.rate_limit_enabled and not (
            ...
            # Short-circuit 429 for resources/read when over limit to avoid long streaming stalls
            if kind == "resources":
...
def build_http_app(settings: Settings, server=None) -> FastAPI:
    ...
    # Build MCP HTTP sub-app with stateless mode (confirmed working with FastAPI lifespan)
    ...
    # Ensure authentication runs BEFORE RBAC/rate limit to return 401 when unauthenticated.
    ...
    with contextlib.suppress(Exception):
        fastapi_app.add_middleware(
            ...
            base_paths=[
                (settings.http.path or "/mcp").rstrip("/") or "/",
    ...
    # Mount FastMCP HTTP app using native implementation
    ...
    # Mount MCP HTTP app at the base path
    ...
    def _register_mail_ui() -> None:
        ...
        async def mail_unified_inbox(
            ...
            lang_sel = (lang or request.query_params.get("lang") or "en").lower()
        ...
        @fastapi_app.get("/mail/unified-inbox-lite", response_class=HTMLResponse)
        async def unified_inbox_lite() -> HTMLResponse:
            return HTMLResponse(
                """
                <!doctype html>
                <html lang=\"en\">
                <head>
                  ...
                  <title>Unified Inbox Lite</title>
                  <style>
                    html,body{height:100%}
                    body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;margin:0;display:flex;align-items:center;justify-content:center;background:#fff;color:#111}
                    .wrap{max-width:640px;padding:24px;border:1px solid #ddd;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.06)}
        ...
        async def mail_project(
            ...
            async with get_session() as session:
                ...
                agents_q = await session.execute(
                    text(
                        "SELECT id, name, program, model, task_description, task_summary, skills, primary_model "
                    ...
                    if isinstance(raw_skills, str):
                        try:
                            ...
                            if isinstance(parsed, list):
                                skills = [str(s) for s in parsed if s is not None]
                        except Exception:
                            skills = []
                    elif isinstance(raw_skills, list):
                        skills = [str(s) for s in raw_skills if s is not None]
                    agents.append(
                        {
                            "id": r[0],
                            "name": r[1],
                            "program": r[2],
                            "model": r[3],
                            "task_description": r[4] or "",
                            "task_summary": r[5] or "",
                            "skills": skills,
                            "primary_model": r[7] or r[3],
                        }
                    )
...
def app() -> FastAPI:
    """ASGI factory for Uvicorn (--factory not required).
