...
7. すべての監査応答は末尾に **Self-check** を付し、(a)実測のみで記載したか (b)テスト結果/未実施理由を明記したか (c) レーン別上限（A=3ファイル/50行、B=原則130行・最大200行/5ファイル、>200はリード承認＋人間確認、400超禁止、C=ロジック不変で無制限）に言及したか (d) shadow audit への言及があるか を Yes/No で記載する。
...
5. プロジェクト固有の GitOps 制約（レーン別上限: A=3 files/50 lines、B=原則130行・最大200行/5 files、>200はリード承認＋人間確認、400超禁止、C=ロジック不変・行数無制限）が  
...
2. **Change 分析**
  ...
  * GitOps diff 制約（レーン別: A=3 files/50 lines、B=原則130行・最大200行/5 files、>200はリード承認＋人間確認、400超禁止、C=ロジック不変・行数無制限）に違反していないか確認する。
3. **リスク評価**
