...
7. すべての監査応答は末尾に **Self-check** を付し、(a)実測のみで記載したか (b)テスト結果/未実施理由を明記したか (c) レーン別上限（A=3ファイル/50行、B=原則130行・最大200行/5ファイル、>200はリード承認＋人間確認、400超禁止、C=ロジック不変で無制限、T=tests/** 限定で20ファイル/800行・テスト失敗時はロールバック必須）に言及したか (d) shadow audit への言及があるか を Yes/No で記載する。
...
- プロジェクト完了フェーズでは、`codexcli_git_ops_policy.md`・`project_rules.yaml`・`WORK_rules.yaml`・`AUDIT_rules.yaml`・`APPROVALS.md` で定義された条件を満たす HEAD に対してのみ、AUDIT の `Status` が auto-push / 将来の auto-merge のゲートとして扱われる。
...
5. プロジェクト固有の GitOps 制約（レーン別上限: A=3 files/50 lines、B=原則130行・最大200行/5 files、>200はリード承認＋人間確認、400超禁止、C=ロジック不変・行数無制限、T=tests/**限定で20 files/800 lines・本番コード混在はA/B違反・テスト失敗時ロールバック必須）が  
  `codexcli_git_ops_policy.md` やルールファイルに定められている場合、  
  監査AIは diff が該当レーンの上限に違反していないかを確認する。  
  - 制約違反があれば少なくとも Major として報告し、  
  - Tレーンの PR は description 冒頭に `lane: T` が明記されているか確認し、未記載の場合は注意喚起する。  
6. goal_id / legacy_feature_ref の有無と `closes:<goal_id>` 記載を確認し、DoD達成可否を判定する。欠落は Major。
7. scope drift 検査: diff が goal_id に紐づく paths 外や out_of_scope を侵していないか確認。違反は Major 以上。
8. WIP=1 準拠: Active goal が複数ある場合は Major とし、PARKED goal の有無も確認する。
...
2. **Change 分析**
  ...
  * GitOps diff 制約（レーン別: A=3 files/50 lines、B=原則130行・最大200行/5 files、>200はリード承認＋人間確認、400超禁止、C=ロジック不変・行数無制限、T=tests/**限定で20 files/800 lines・本番コード混在はA/B違反・テスト失敗時ロールバック必須）に違反していないか確認する。
3. **リスク評価**
